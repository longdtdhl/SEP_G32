...
public class DoctorApiService : ApiServiceBase, IDoctorApiService
{
    ...
    public async Task<(List<DoctorListItemDto> Data, PaginationDto? Pagination, string? Error)> GetAllAsync(DoctorFilterDto? filter = null)
    {
        ...
        if (filter != null)
        {
            ...
            if (filter.SpecializationId.HasValue) parts.Add($"specializationId={filter.SpecializationId.Value}");
    ...
    {
        var (data, _, error) = await GetAsync<List<TimeSlotDto>>($"{ApiRoutes.Doctors}/{doctorId}/schedule?date={date:yyyy-MM-dd}");
