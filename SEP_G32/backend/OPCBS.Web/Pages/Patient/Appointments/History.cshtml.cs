...
public class HistoryModel : PageModel
{
    ...
    public HistoryModel(IAppointmentApiService service) { _service = service; }
    ...
    public async Task OnGetAsync(int page = 1, string? status = null, string? doctor = null, string? date = null)
    {
        ...
        try
        {
            ...
            var (allData, _, error) = await _service.GetMyAppointmentsAsync(new AppointmentFilterDto { View = "history", Page = 1, PageSize = 9999 });
            if (allData != null)
            {
                ...
                {
                    query = query.Where(a => string.Equals(a.StatusText, Status, StringComparison.OrdinalIgnoreCase));
                }
