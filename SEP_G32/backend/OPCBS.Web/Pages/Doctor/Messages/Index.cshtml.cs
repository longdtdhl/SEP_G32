...
namespace OPCBS.Web.Pages.Doctor.Messages;
...
public class IndexModel : PageModel
{
    ...
    {
        // If patientId provided (from Appointments), create or get conversation
        if (patientId.HasValue && !conversationId.HasValue)
        {
            var (convo, convoError) = await _messagingService.GetOrCreateConversationByPatientAsync(patientId.Value);
            if (convo != null)
                conversationId = convo.Id;
            else
                ErrorMessage = convoError ?? "Could not start conversation with this patient.";
        }
        ...
        if (conversationId.HasValue)
        {
            ...
            var (msgs, _) = await _messagingService.GetMessagesAsync(conversationId.Value);
            Messages = msgs;
